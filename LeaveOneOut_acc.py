...
#    load data from the file
    filename = r'data/train_data_split_amp.mat'
    
    tmp = sio.loadmat(filename)
    xdata=np.array(tmp['train_data'])
    label=np.array(tmp['train_activity_label'])
    subIdx=np.array(tmp['train_location_label'])
...
#   ydata contains the label of samples   
    ...
    print('the ydata is:',ydata.shape)
